...
class EnvironmentInitializer(smach.State):
    ...
    def execute(self, userdata):
        ...
        Returns:
            INITIALIZATION_COMPLETE(str): transits to STATE_NORMAL which is a nested state_machine
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                if self._helper.controller_client.is_done():
                    self._knowledge.relocate(target, robot_position)
...
class ChargeManagement(State):
    ...
    def execute(self, userdata):
        """
        This function manages the transition from the `RECHARGING_BATTERY` state to the `STATE_NORMAL` state, 
        ...
        Returns:
            RECHARGING_COMPLETE(str): transition to STATE_NORMAL
